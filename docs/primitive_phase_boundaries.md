# Primitive Phase Boundaries Contract

本文件是 `dig / carry / dump / return` 四 primitive 的基础 ownership 契约，
约束录制、离线 builder、训练和 rollout planner 对动作阶段的共同理解。

它最早来自 V2.2 四 primitive 稳线，但不是一次 V2.2 实验记录。当前 V2.4 /
V2.4.5 数据链仍通过 `tb-build-primitives-v2_2` 和不同
`boundary_profile` 复用这层 ownership 结构，例如主线的
`v2_4_5_spatial_mass` 和 new-env pilot 的 `v2_2_effect_release_fallback`。

本文件的核心目标是避免上一段 skill 学到下一段 skill 的动作，尤其避免
`carry` 学会提前 curl-out/release。具体阈值仍以当前 boundary profile、QC
合同和配置为准。
