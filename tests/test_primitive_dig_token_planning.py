...
def _ports(
    *,
    ...
    state: dict[str, object] = {
        ...
        "pending_state_exemplar_ids": ["ex_a", "ex_b"],
        "pending_state_exemplar_distance": 1.25,
        "pending_profile": pending_profile,
        "active_corridor": active_corridor,
        "dig_cut_tokens": _token(DIG_CUT_TOKEN_DIM, 0.0),
        "dig_cut_source": "old_cut",
        "dig_cut_fallback": "old_cut_reason",
        "dig_cut_in_prior": False,
        "depth_source": "old_depth",
        "depth_fallback": "old_depth_reason",
        "active_state_exemplar_ids": [],
        "active_state_exemplar_distance": float("nan"),
        "active_state_exemplar_profile_token": None,
        "active_corridor_id": -1,
        "last_selected_corridor_id": -1,
        "current_payload_gain": 99.0,
        "cycle_start_deposit": -1.0,
    }
    ...
    def set_active_state_exemplar(
        ...
        events.append(f"set_active_exemplar:{exemplar_ids}:{distance}")
        state["active_state_exemplar_ids"] = list(exemplar_ids)
        state["active_state_exemplar_distance"] = float(distance)
        state["active_state_exemplar_profile_token"] = (
            None
    ...
    ports = PrimitiveDigTokenPlanningPorts(
        ...
        get_pending_dig_cut_cycle_id=lambda: int(state["pending_cycle_id"]),
        get_pending_dig_cut_tokens=lambda: state["pending_tokens"],
        get_pending_dig_cut_raw_fields=lambda: state["pending_raw_fields"],
        get_pending_dig_cut_corridor_id=lambda: int(state["pending_corridor_id"]),
        get_pending_dig_state_exemplar_ids=lambda: list(
            state["pending_state_exemplar_ids"]
        ),
        get_pending_dig_state_exemplar_distance=lambda: float(
            state["pending_state_exemplar_distance"]
        ),
        get_pending_dig_depth_profile_tokens=lambda: state["pending_profile"],
        get_dig_cut_tokens=lambda: state["dig_cut_tokens"],
        set_dig_cut_token_source=lambda value: set_value(
            "dig_cut_source",
            str(value),
        ),
        set_dig_cut_fallback_reason=lambda value: set_value(
            "dig_cut_fallback",
            str(value),
        ),
        set_dig_cut_token_in_prior_p10_p90=lambda value: set_value(
            "dig_cut_in_prior",
            bool(value),
        ),
        set_dig_depth_profile_token_source=lambda value: set_value(
            "depth_source",
            str(value),
        ),
        set_dig_depth_profile_fallback_reason=lambda value: set_value(
            "depth_fallback",
            str(value),
        ),
        ...
        active_coverage_corridor=lambda: state["active_corridor"],
        coverage_corridor_by_id=lambda corridor_id: corridors.get(int(corridor_id)),
        set_coverage_active_corridor_id=lambda value: set_value(
            "active_corridor_id",
            int(value),
        ),
        set_coverage_last_selected_corridor_id=lambda value: set_value(
            "last_selected_corridor_id",
            int(value),
        ),
        set_coverage_current_payload_gain_kg=lambda value: set_value(
            "current_payload_gain",
            float(value),
        ),
        set_coverage_cycle_start_deposit_kg=lambda value: set_value(
            "cycle_start_deposit",
            float(value),
        ),
        set_coverage_active_state_exemplar=set_active_state_exemplar,
        get_coverage_active_state_exemplar_profile_token=(
            lambda: state["active_state_exemplar_profile_token"]
        ),
    )
...
def test_pending_return_target_route_writes_coverage_state_and_copies() -> None:
    ...
    assert events == [
        "set_dig_cut_fallback:",
        "plan_pending:1.5:{'operator_entry_x_m': 9.0}",
        "set_active_corridor_id:12",
        "set_last_selected_corridor_id:12",
        "set_current_payload_gain:0.0",
        "deposited:obs",
        "set_cycle_start_deposit:42.0",
        "set_active_exemplar:['ex_a', 'ex_b']:1.25",
        "set_dig_cut_source:pending_return_target",
        "set_dig_cut_fallback:",
        "set_dig_cut_in_prior:True",
    ]
    assert state["active_corridor_id"] == 12
    assert state["last_selected_corridor_id"] == 12
    assert state["current_payload_gain"] == 0.0
    assert state["cycle_start_deposit"] == 42.0
    assert state["active_state_exemplar_ids"] == ["ex_a", "ex_b"]
    assert state["active_state_exemplar_distance"] == 1.25
    assert state["active_state_exemplar_profile_token"] is not state["pending_profile"]
...
def test_conservative_and_operator_prior_routes_apply_dig_cut_plan() -> None:
    ...
    assert events == [
        ...
        "set_dig_cut_source:conservative_pose",
        "set_dig_cut_fallback:",
        "set_dig_cut_in_prior:False",
    ]
    assert state["dig_cut_source"] == "conservative_pose"
    ...
    assert events == [
        ...
        "set_dig_cut_source:operator_prior_pose_clamped",
        "set_dig_cut_fallback:",
        "set_dig_cut_in_prior:True",
    ]
    assert state["dig_cut_source"] == "operator_prior_pose_clamped"
...
def test_operator_prior_fallback_and_disabled_reraise() -> None:
    ...
    assert events == [
        ...
        "set_dig_cut_source:fallback_conservative_pose",
        "set_dig_cut_fallback:prior failed",
        "set_dig_cut_in_prior:False",
    ]
...
def test_coverage_routes_use_existing_coverage_raw_field_builder(mode: str) -> None:
    ...
    assert events == [
        ...
        "plan_raw:operator_prior_coverage:22.5:",
        "plan_raw:operator_prior_coverage:22.5:",
        "set_dig_cut_source:operator_prior_coverage",
        "set_dig_cut_fallback:",
        "set_dig_cut_in_prior:True",
    ]
...
def test_coverage_route_fallback_and_unsupported_mode_error() -> None:
    ...
    assert events[-4:] == [
        ...
        "set_dig_cut_source:fallback_conservative_pose",
        "set_dig_cut_fallback:coverage failed",
        "set_dig_cut_in_prior:False",
    ]
    np.testing.assert_allclose(token, _token(DIG_CUT_TOKEN_DIM, 4.0))
...
def test_dig_cut_apply_and_depth_profile_success_error_and_helpers() -> None:
    ...
    assert events == [
        ...
        "set_depth_source:depth_source",
        "set_depth_fallback:",
    ]
    assert state["depth_source"] == "depth_source"
    ...
    assert error_state["depth_source"] == "missing_required_prior"
    assert error_state["depth_fallback"] == "missing dig_cut_prior"
    assert error_events[-2:] == [
        "set_depth_source:missing_required_prior",
        "set_depth_fallback:missing dig_cut_prior",
    ]
...
def test_raw_fields_priority_and_token_decoding() -> None:
    ...
    assert state["pending_raw_fields"] == pending_raw
    ...
    ports, state, events, _, _ = _ports()
    state["dig_cut_tokens"] = dig_token
    raw = PrimitiveDigTokenPlanningService.from_ports(
        ports
    ).dig_depth_profile_raw_fields({"id": "obs", "pose_x": 1, "pose_y": 2, "pose_z": 3})
