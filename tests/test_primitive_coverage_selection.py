...
from testbed.planner.primitive_coverage import CoverageSelectionService
...
def test_coverage_selection_service_matches_planner_facade_candidate_scores() -> None:
    ...
    facts_by_id = policy._coverage_selection_facts(obs, service_corridors)
    recent_row_reference = next(
        corridor
        for corridor in service_corridors
        if int(corridor.corridor_id) == policy._coverage_last_selected_corridor_id
    )
    service_result = CoverageSelectionService(
        policy._coverage_selection_config(),
    ).select(
        service_corridors,
    ...
    selected = policy._select_coverage_corridor(obs)
