"""CLI for the exactly-once observe-only wall-contact rollout."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="tb-wall-contact-rollout-probe",
        description=(
            "Prepare, validate, run exactly once, or collect the Strict-18 "
            "observe-only bucket-wall multicycle diagnostic."
        ),
    )
    ...
    prepare.add_argument(
        "--source-rollout", type=Path, default=DEFAULT_SOURCE_ROLLOUT
    )
    prepare.add_argument(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "prepare":
        result = prepare_wall_contact_rollout_probe(
        ...
        result = validate_wall_contact_rollout_probe(args.manifest)
    elif args.command == "run-once":
        result = run_wall_contact_rollout_probe(
            manifest_path=args.manifest,
            cwd=args.cwd,
        )
    else:
        result = collect_wall_contact_rollout_probe(
            manifest_path=args.manifest
        )
    print(
        json.dumps(
