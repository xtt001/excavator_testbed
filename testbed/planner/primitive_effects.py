...
class RequestedEffectApplier:
    ...
    def _apply_one(
        ...
        if isinstance(effect, SwitchSkillEffect):
            ...
            ports.increment_dig_bad_replan_count()
            ...
            ports.set_dump_ready_hold_count(int(effect.value))
        elif isinstance(effect, SetDumpStartDepositedMassFromObservationEffect):
            ports.set_dump_start_deposited_mass(float(ports.deposited_mass(obs)))
        elif isinstance(effect, SetDumpDoneHoldCountEffect):
            ports.set_dump_done_hold_count(int(effect.value))
