...
| Mock AGX server (offline testing) | ✅ implemented |
...
### 2. Start mock AGX server (no Unity needed)

```bash
tb-mock-agx               # listens on 127.0.0.1:9000
```

The mock server speaks the full V0 protocol: GET_INFO / RESET / STEP with raw RGB frames.
`mass_in_bucket` ramps 0 → 2.0 over 200 steps so you can verify the success rule fires.
...
testbed/
  backends/
    ...
    agx/
      ...
      client.py               ← TCP client, strict step-ack enforcement
      backend.py              ← AGXSimBackend(SimBackend) — primary backend
      mock_server.py          ← Offline test server (tb-mock-agx)
...
|---|---|---|
| `GET_INFO_REQ/RESP` | Python → Unity / Unity → Python | JSON: dt, control_hz, dims, cameras |
...
| `STEP_RESP` | Unity → Python | step_id, qpos[3], qvel[4], env_state[M], reward, fpv image |

**Action vector:** `[swing_speed_cmd, boom_speed_cmd, stick_speed_cmd, bucket_speed_cmd]` — normalized `[-1, 1]`

**Observation:**
- `qpos (3,)` — `[boom, stick, bucket]` position_norm `[0, 1]`
